...
func Constructor_() MyLinkedList2 {
	rear := &Node{
		Val:  -1,
		Next: nil,
		Pre:  nil,
...
func (this *MyLinkedList2) AddAtIndex(index int, val int) {
	...
	node := &Node{
		Val: val,
		//node.Next = MyLinkedList2[index]
		Next: head,
		//node.Pre = MyLinkedList2[index-1]
		Pre: head.Pre,
	}
	//MyLinkedList2[index-1].Next = node
	head.Pre.Next = node
	//MyLinkedList2[index].Pre = node
	head.Pre = node
	//以上两步不能反
}
...
func (this *MyLinkedList2) DeleteAtIndex(index int) {
	...
	//验证index有效
	if index == 0 {
		//MyLinkedList2[index].Pre = index[index-2]
